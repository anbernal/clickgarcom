...
async function loadWallet() {
    ...
    try {
        // Obter Saldo do Go-Core
        const res = await api.get('/wallet/balance');
        if (!res) throw new Error("Erro ao carregar saldo");

        const balance = res.wallet_balance || 0;
        const plan = res.billing_plan === 'pre_paid' ? 'Pré-Pago' : 'Pós-Pago';

        container.innerHTML = `
            <div style="max-width: 650px; margin: 0 auto; background: var(--card-bg); padding: 30px; border-radius: var(--border-radius-lg); box-shadow: var(--shadow-md);">
                <div style="text-align: center; margin-bottom: 24px;">
                    ...
                    <div style="font-size: 14px; color: var(--muted);">Saldo Mínimo para Operar o WhatsApp Sem Quedas: R$ 0,00</div>
                </div>

                <hr style="border: 0; border-top: 1px solid var(--border-color); margin: 30px 0;">

                <h3 style="margin-bottom: 16px; font-family: var(--font-heading);">Recarregar Saldo (PIX Instantâneo)</h3>
                <form id="wallet-recharge-form" style="display: flex; flex-direction: column; gap: 16px;">
                    ...
                    <div style="display:flex; gap: 16px;">
                        ...
                        <div style="display:flex; flex-direction:column; gap:6px; flex:1;">
                            ...
                            <input type="text" id="wallet-name" style="padding:12px; border:1px solid var(--border-color); border-radius:6px;" placeholder="João Silva" required>
                        </div>
                    </div>
                    <div style="display:flex; flex-direction:column; gap:6px;">
                ...
                <div id="wallet-qr-container" style="display: none;text-align: center; margin-top: 30px; background: #f8f9fa; padding: 30px; border-radius: 12px; border: 1px dashed var(--border-color);">
                    ...
                    <p style="color: var(--success); font-weight: 600; font-size: 14px; margin-top:15px;">Aguardando pagamento... O saldo será atualizado automaticamente assim que compensar.</p>
                </div>
            </div>
        `;
        ...
        document.getElementById('wallet-recharge-form').addEventListener('submit', async (e) => {
            ...
            // Fake UUID used to hack Webhook logic inside Go-Core representing Wallet Recharge
            const payload = {
