...
func (c *Consumer) Consume(queueName string, handler MessageHandler) error {
	// Declarar fila antes de consumir para garantir que existe
	...
	go func() {
		for msg := range msgs {
			...
			if err := handler(ctx, msg.Body); err != nil {
				...
				// TEMP FIX: Ack on error to flush queue
				c.logger.Warn("ACKing bad message to flush queue", zap.String("message_id", msg.MessageId))
				msg.Ack(false)
				continue
