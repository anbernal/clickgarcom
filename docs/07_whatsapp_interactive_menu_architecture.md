...
- categorias em `apps/tenant-admin/api/src/entities/menu-category.entity.ts`
- itens com `image_url` e `destination` em `apps/tenant-admin/api/src/entities/menu-item.entity.ts`
- dominio do menu em `platform/core-backend/internal/domain/menu/entity.go`
- criacao de pedidos com varios itens em `platform/core-backend/internal/application/create_order.go`
- KDS separado por `destination` em `apps/tenant-admin/web/public/js/kds.js`

O problema atual e que o `Order` ainda tem um `destination` unico em `platform/core-backend/internal/domain/order/entity.go`.
...
Hoje o KDS ja separa por `destination` em `apps/tenant-admin/web/public/js/kds.js`.
