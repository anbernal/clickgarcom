...
├── apps/
│   ├── tenant-admin/
│   │   ├── api/               # NestJS API reutilizavel por web atual e futuro mobile
│   │   └── web/               # Frontend admin/KDS/checkout
│   └── super-admin/
│       └── web/               # Painel global estatico
│
├── platform/
│   └── core-backend/          # Backend Go: API, workers, websocket e integrações
│       ├── cmd/
│       │   ├── api/
│       │   ├── worker/
│       │   ├── outbox-worker/
...
- `connected` - Evento de controle ao conectar no WebSocket
- `order.created` - Novo pedido operacional criado
- `order.status_changed` - Mudança de status operacional do pedido

Contrato detalhado:

- [`docs/kds-websocket-contract.md`](kds-websocket-contract.md)
...
### Variáveis de Ambiente (`platform/core-backend/.env`)
