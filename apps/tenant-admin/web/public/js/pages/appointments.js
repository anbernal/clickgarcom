...
async function saveAppointmentProfessional(id) {
    ...
    if (!name) { document.getElementById('appointment-professional-name')?.focus(); showToast('Informe o nome do profissional.', 'error'); return; }
    if (!role) { document.getElementById('appointment-professional-role')?.focus(); showToast('Informe a função do profissional.', 'error'); return; }
    if (!services.length) { document.querySelector('input[name="appointment-professional-service"]')?.focus(); showToast('Marque pelo menos um serviço realizado por este profissional.', 'error'); return; }
