...
function renderDeliverySettingsModal(settings, capacity = null) {
    ...
    return `<div class="modal-header"><div><h3>Configuração da operação Delivery</h3><div class="modal-header-subtitle">Área, taxas e regras de aceite automático.${settingsVersion ? ` Última alteração: ${escapeHTML(new Date(settingsVersion).toLocaleString('pt-BR'))}` : ''}</div></div><button class="modal-close" onclick="closeModal()">✕</button></div><div class="modal-body delivery-form" id="delivery-settings-form">
        ...
        <section class="delivery-form-section"><div class="delivery-form-section-head"><div><h4>Origem e área de atendimento</h4><p>Use as coordenadas confirmadas do restaurante e defina o raio máximo.</p></div></div><div class="delivery-form-grid delivery-form-grid--3"><div class="form-group"><label for="delivery-setting-lat">Latitude</label><input id="delivery-setting-lat" type="number" step="0.000001" min="-90" max="90" value="${settings.origin?.lat ?? ''}" placeholder="-23.550520"></div><div class="form-group"><label for="delivery-setting-lng">Longitude</label><input id="delivery-setting-lng" type="number" step="0.000001" min="-180" max="180" value="${settings.origin?.lng ?? ''}" placeholder="-46.633308"></div><div class="form-group"><label for="delivery-setting-radius">Raio (km)</label><input id="delivery-setting-radius" type="number" min="0.1" max="500" step="0.1" value="${Number(settings.service_area?.radius_km || 8)}"></div></div></section>
...
function validateDeliverySettings(payload) {
    if (payload.enabled && (!Number.isFinite(payload.origin_lat) || !Number.isFinite(payload.origin_lng))) return 'Informe e confirme latitude e longitude antes de ativar o módulo.';
