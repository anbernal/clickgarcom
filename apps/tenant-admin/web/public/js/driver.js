...
function activateDriverLocation() { if (!navigator.geolocation) { driverToast('Este aparelho não oferece localização.'); return; } navigator.geolocation.getCurrentPosition(() => { driverState.locationEnabled = true; driverState.locationErrorShown = false; driverSyncLocationTracking(); renderDriverApp(); driverToast('Localização ativada para esta entrega.'); }, () => { driverState.locationEnabled = false; driverState.locationErrorShown = true; driverToast('Permita a localização nas configurações do aparelho para acompanhar a rota.'); renderDriverApp(); }, { enableHighAccuracy:true, maximumAge:10000, timeout:15000 }); }
function driverSyncLocationTracking() { const active = driverLocationDelivery(); if (!DRIVER_API_ENABLED || !driverState.locationEnabled || !navigator.geolocation || !active) { if (driverState.locationWatchId !== null) { navigator.geolocation?.clearWatch(driverState.locationWatchId); driverState.locationWatchId = null; } return; } if (driverState.locationWatchId !== null) return; driverState.locationWatchId = navigator.geolocation.watchPosition(position => { if (Date.now() - driverState.locationLastSentAt < 10000) return; driverState.locationLastSentAt = Date.now(); const coords = position.coords; const point = { event_id: crypto.randomUUID(), lat: Number(coords.latitude.toFixed(7)), lng: Number(coords.longitude.toFixed(7)), accuracy_m: coords.accuracy == null ? undefined : Number(coords.accuracy.toFixed(1)), speed_mps: coords.speed == null ? undefined : Number(coords.speed.toFixed(2)), heading_deg: coords.heading == null ? undefined : Number(coords.heading.toFixed(1)), recorded_at: new Date().toISOString() }; if (!driverState.online) { driverQueueLocation(active.delivery_id, point); renderDriverApp(); return; } driverGateway.location(active.delivery_id, [point]).catch(() => { driverQueueLocation(active.delivery_id, point); renderDriverApp(); }); }, () => { driverState.locationEnabled = false; if (!driverState.locationErrorShown) { driverState.locationErrorShown = true; driverToast('Permita a localização para atualizar sua rota.'); } renderDriverApp(); }, { enableHighAccuracy:true, maximumAge:10000, timeout:15000 }); flushDriverPendingLocations(); }
