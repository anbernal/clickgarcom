...
const driverGateway = {
    ...
    async history() {
        ...
        return [{ id:'history-1', delivery_code:'584210', completed_at:driverNow(-52), neighborhood:'Centro', duration_minutes:34 }];
    },
...
function renderDriverHistory(){return `<section class="driver-greeting"><span>MEU DIA</span><h1>Entregas concluídas</h1><p>Somente o resumo operacional necessário fica visível no histórico.</p></section><div class="driver-history">${driverState.history.length?driverState.history.map((item)=>`<article><span>✓</span><div><strong>#${driverEscape(item.delivery_code)} · ${driverEscape(item.neighborhood||'Destino')}</strong><small>${new Date(item.completed_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})} · ${Number(item.duration_minutes||0)} min</small></div></article>`).join(''):'<div class="driver-empty"><span>▤</span><strong>Nenhuma entrega concluída hoje</strong></div>'}</div>`;}
