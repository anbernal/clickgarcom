...
function renderDriverApp(){const driver=driverState.session?.driver||{};const tenant=driverState.session?.tenant||{};const route=driverState.assignments.filter((item)=>item.status!=='DELIVERED');const pending=driverPendingCount();const active=driverLocationDelivery();const locationAction=active&&DRIVER_API_ENABLED?(driverState.locationWatchId===null?'<button class="driver-location-activate" onclick="activateDriverLocation()">Ativar localização</button>':'<span class="driver-location-active">Ativa durante a rota</span>'):'';document.getElementById('driver-app').innerHTML=`<div class="driver-shell"><header class="driver-header"><div class="driver-brand"><span class="driver-logo">${driverLogo()}</span><div><strong>${driverEscape(tenant.name||'Restaurante')}</strong><span>${driverEscape(driver.name||'Motoboy')} · ${driverEscape(driver.plate||'')}</span></div></div><span class="driver-online"><i></i>${driverState.online?'Online':'Offline'}</span></header>${driverState.online?'':'<div class="driver-offline">Sem internet. A fila exibida pode estar desatualizada.</div>'}${pending?`<div class="driver-sync-banner">⟳ ${pending} atualização(ões) aguardando sincronização.</div>`:''}${active?`<div class="driver-location-banner"><span>📍 Ative a localização para atualizar o cliente durante a entrega.</span>${locationAction}</div>`:''}<main class="driver-main">${!DRIVER_API_ENABLED?'<div class="driver-preview"><strong>Prévia funcional.</strong> A autenticação HttpOnly, os eventos e as ações reais serão ligados pelo adapter.</div>':''}${driverState.screen==='queue'?renderDriverQueue(route):driverState.screen==='history'?renderDriverHistory() : renderDriverProfile()}</main>${renderDriverNav()}</div>`;}
