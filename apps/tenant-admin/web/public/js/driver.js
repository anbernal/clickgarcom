...
const driverState = { screen: 'queue', session: null, online: navigator.onLine, shiftOpen: true, assignments: [], history: [], timer: null, busy: new Set(), demoQueueInitialized: false, locationWatchId: null, locationLastSentAt: 0, locationErrorShown: false, flushing: false };
...
function driverSyncLocationTracking() { const active = driverLocationDelivery(); if (!DRIVER_API_ENABLED || !navigator.geolocation || !active) { if (driverState.locationWatchId !== null) { navigator.geolocation?.clearWatch(driverState.locationWatchId); driverState.locationWatchId = null; } return; } if (driverState.locationWatchId !== null) return; driverState.locationWatchId = navigator.geolocation.watchPosition(position => { if (Date.now() - driverState.locationLastSentAt < 10000) return; driverState.locationLastSentAt = Date.now(); const coords = position.coords; const point = { event_id: crypto.randomUUID(), lat: Number(coords.latitude.toFixed(7)), lng: Number(coords.longitude.toFixed(7)), accuracy_m: coords.accuracy == null ? undefined : Number(coords.accuracy.toFixed(1)), speed_mps: coords.speed == null ? undefined : Number(coords.speed.toFixed(2)), heading_deg: coords.heading == null ? undefined : Number(coords.heading.toFixed(1)), recorded_at: new Date().toISOString() }; if (!driverState.online) { driverQueueLocation(active.delivery_id, point); renderDriverApp(); return; } driverGateway.location(active.delivery_id, [point]).catch(() => { driverQueueLocation(active.delivery_id, point); renderDriverApp(); }); }, () => { if (!driverState.locationErrorShown) { driverState.locationErrorShown = true; driverToast('Permita a localização para atualizar sua rota.'); } }, { enableHighAccuracy:true, maximumAge:10000, timeout:15000 }); flushDriverPendingLocations(); }
...
function renderDriverApp(){const driver=driverState.session?.driver||{};const tenant=driverState.session?.tenant||{};const route=driverState.assignments.filter((item)=>item.status!=='DELIVERED');const pending=driverPendingCount();const active=driverLocationDelivery();document.getElementById('driver-app').innerHTML=`<div class="driver-shell"><header class="driver-header"><div class="driver-brand"><span class="driver-logo">${driverLogo()}</span><div><strong>${driverEscape(tenant.name||'Restaurante')}</strong><span>${driverEscape(driver.name||'Motoboy')} · ${driverEscape(driver.plate||'')}</span></div></div><span class="driver-online"><i></i>${driverState.online?'Online':'Offline'}</span></header>${driverState.online?'':'<div class="driver-offline">Sem internet. A fila exibida pode estar desatualizada.</div>'}${pending?`<div class="driver-sync-banner">⟳ ${pending} atualização(ões) aguardando sincronização.</div>`:''}${active?'<div class="driver-location-banner">📍 Localização compartilhada durante a entrega.</div>':''}<main class="driver-main">${!DRIVER_API_ENABLED?'<div class="driver-preview"><strong>Prévia funcional.</strong> A autenticação HttpOnly, os eventos e as ações reais serão ligados pelo adapter.</div>':''}${driverState.screen==='queue'?renderDriverQueue(route):driverState.screen==='history'?renderDriverHistory() : renderDriverProfile()}</main>${renderDriverNav()}</div>`;}
