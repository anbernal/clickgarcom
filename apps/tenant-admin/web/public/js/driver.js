...
const driverState = { screen: 'queue', session: null, online: navigator.onLine, shiftOpen: true, assignments: [], history: [], timer: null, busy: new Set(), demoQueueInitialized: false, locationEnabled: false, locationWatchId: null, locationLastSentAt: 0, locationErrorShown: false, flushing: false };
...
const driverGateway = {
    ...
    async location(deliveryId, points) {
        if (DRIVER_API_ENABLED) {
            const response = await fetch(`/admin/api/public/delivery/drivers/deliveries/${encodeURIComponent(deliveryId)}/location`, { method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body:JSON.stringify({ points }) });
            if (!response.ok) { const error = await response.json().catch(()=>({})); throw new Error(error.message || 'Não foi possível sincronizar a localização.'); }
            return response.json();
        }
        return { accepted: points, rejected: [] };
    },
    async shift(open) {
        if (DRIVER_API_ENABLED) {
...
function driverStorageKey(name) { return `clickgarcom_driver_${name}_${driverState.session?.driver?.profile_id || driverState.session?.driver?.id || 'unknown'}`; }
function driverReadStorage(name, fallback = []) { try { const value = JSON.parse(localStorage.getItem(driverStorageKey(name)) || 'null'); return Array.isArray(value) ? value : fallback; } catch (_) { return fallback; } }
function driverWriteStorage(name, value) { try { localStorage.setItem(driverStorageKey(name), JSON.stringify(value)); } catch (_) {} }
function driverLocationPreferenceKey() { return `clickgarcom_driver_location_enabled_${driverState.session?.driver?.profile_id || driverState.session?.driver?.id || 'unknown'}`; }
function driverRestoreLocationPreference() { try { return sessionStorage.getItem(driverLocationPreferenceKey()) === '1'; } catch (_) { return false; } }
function driverSetLocationEnabled(enabled) { driverState.locationEnabled = enabled; try { const key = driverLocationPreferenceKey(); if (enabled) sessionStorage.setItem(key, '1'); else sessionStorage.removeItem(key); } catch (_) {} }
function driverPendingCommands() { return driverReadStorage('pending_commands'); }
function driverPendingLocations() { return driverReadStorage('pending_locations'); }
function driverPendingCount() { return driverPendingCommands().length + driverPendingLocations().length; }
function driverQueueCommandOffline(deliveryId, command, body) { const pending = driverPendingCommands(); pending.push({ id: crypto.randomUUID(), delivery_id: deliveryId, command, body, created_at: new Date().toISOString() }); driverWriteStorage('pending_commands', pending.slice(-40)); }
function driverQueueLocation(deliveryId, point) { const pending = driverPendingLocations(); pending.push({ delivery_id: deliveryId, point }); driverWriteStorage('pending_locations', pending.slice(-120)); }
function driverApplyLocalCommand(deliveryId, command) { const status = { pickup:'PICKED_UP', start:'IN_TRANSIT', arrive:'ARRIVED', incident:'OCCURRENCE' }[command]; if (!status) return; driverState.assignments = driverState.assignments.map(item => item.delivery_id === deliveryId ? { ...item, status, version:Number(item.version || 1) + 1, updated_at:driverNow() } : item); renderDriverApp(); driverSyncLocationTracking(); }
async function flushDriverPendingCommands() { if (!DRIVER_API_ENABLED || !driverState.online || driverState.flushing) return; driverState.flushing = true; try { const pending = driverPendingCommands(); const remaining = []; for (const item of pending) { try { await driverGateway.command(item.delivery_id, item.command, item.body || {}); } catch (_) { remaining.push(item); break; } } driverWriteStorage('pending_commands', remaining); if (pending.length !== remaining.length) await loadDriverQueue(); } finally { driverState.flushing = false; renderDriverApp(); } }
async function flushDriverPendingLocations() { if (!DRIVER_API_ENABLED || !driverState.online) return; const pending = driverPendingLocations(); if (!pending.length) return; const grouped = new Map(); pending.forEach(item => { if (!grouped.has(item.delivery_id)) grouped.set(item.delivery_id, []); grouped.get(item.delivery_id).push(item.point); }); for (const [deliveryId, points] of grouped) { try { await driverGateway.location(deliveryId, points); const ids = new Set(points.map(point => point.event_id)); driverWriteStorage('pending_locations', driverPendingLocations().filter(item => !ids.has(item.point?.event_id))); } catch (_) { break; } } renderDriverApp(); }
function driverLocationDelivery() { return driverState.assignments.find(item => ['PICKED_UP','IN_TRANSIT','ARRIVED'].includes(item.status)); }
function driverRecordLocation(position, force = false) { const active = driverLocationDelivery(); if (!active || (!force && Date.now() - driverState.locationLastSentAt < 10000)) return; driverState.locationLastSentAt = Date.now(); const coords = position.coords; const point = { event_id: crypto.randomUUID(), lat: Number(coords.latitude.toFixed(7)), lng: Number(coords.longitude.toFixed(7)), accuracy_m: coords.accuracy == null ? undefined : Number(coords.accuracy.toFixed(1)), speed_mps: coords.speed == null ? undefined : Number(coords.speed.toFixed(2)), heading_deg: coords.heading == null ? undefined : Number(coords.heading.toFixed(1)), recorded_at: new Date().toISOString() }; if (!driverState.online) { driverQueueLocation(active.delivery_id, point); renderDriverApp(); return; } driverGateway.location(active.delivery_id, [point]).catch(() => { driverQueueLocation(active.delivery_id, point); renderDriverApp(); }); }
function activateDriverLocation() { if (!navigator.geolocation) { driverToast('Este aparelho não oferece localização.'); return; } navigator.geolocation.getCurrentPosition(position => { driverSetLocationEnabled(true); driverState.locationErrorShown = false; driverRecordLocation(position, true); driverSyncLocationTracking(); renderDriverApp(); driverToast('Localização ativada para esta entrega.'); }, () => { driverSetLocationEnabled(false); driverState.locationErrorShown = true; driverToast('Permita a localização nas configurações do aparelho para acompanhar a rota.'); renderDriverApp(); }, { enableHighAccuracy:true, maximumAge:10000, timeout:15000 }); }
function driverSyncLocationTracking() { const active = driverLocationDelivery(); if (!DRIVER_API_ENABLED || !driverState.locationEnabled || !navigator.geolocation || !active) { if (driverState.locationWatchId !== null) { navigator.geolocation?.clearWatch(driverState.locationWatchId); driverState.locationWatchId = null; } return; } if (driverState.locationWatchId !== null) return; driverState.locationWatchId = navigator.geolocation.watchPosition(position => driverRecordLocation(position), () => { driverSetLocationEnabled(false); if (!driverState.locationErrorShown) { driverState.locationErrorShown = true; driverToast('Permita a localização para atualizar sua rota.'); } renderDriverApp(); }, { enableHighAccuracy:true, maximumAge:10000, timeout:15000 }); flushDriverPendingLocations(); }
...
async function loadDriverQueue() { try { const [queue, history] = await Promise.all([driverGateway.queue(), driverGateway.history().catch(()=>driverState.history)]); driverState.assignments = queue; driverState.history = history || []; if (!driverState.locationEnabled && driverRestoreLocationPreference()) driverState.locationEnabled = true; driverSyncLocationTracking(); renderDriverApp(); flushDriverPendingLocations(); } catch(error){ driverToast(error.message); renderDriverApp(); } }
...
function renderDriverApp(){const driver=driverState.session?.driver||{};const tenant=driverState.session?.tenant||{};const route=driverState.assignments.filter((item)=>item.status!=='DELIVERED');const pending=driverPendingCount();const active=driverLocationDelivery();const locationActive=driverState.locationWatchId!==null;const locationAction=active&&DRIVER_API_ENABLED?(locationActive?'<span class="driver-location-active">Ativa durante a rota</span>':'<button class="driver-location-activate" onclick="activateDriverLocation()">Ativar localização</button>'):'';const locationMessage=locationActive?'📍 Localização ativa: o cliente acompanha sua rota.':'📍 Ative a localização para atualizar o cliente durante a entrega.';document.getElementById('driver-app').innerHTML=`<div class="driver-shell"><header class="driver-header"><div class="driver-brand"><span class="driver-logo">${driverLogo()}</span><div><strong>${driverEscape(tenant.name||'Restaurante')}</strong><span>${driverEscape(driver.name||'Motoboy')} · ${driverEscape(driver.plate||'')}</span></div></div><span class="driver-online"><i></i>${driverState.online?'Online':'Offline'}</span></header>${driverState.online?'':'<div class="driver-offline">Sem internet. A fila exibida pode estar desatualizada.</div>'}${pending?`<div class="driver-sync-banner">⟳ ${pending} atualização(ões) aguardando sincronização.</div>`:''}${active?`<div class="driver-location-banner"><span>${locationMessage}</span>${locationAction}</div>`:''}<main class="driver-main">${!DRIVER_API_ENABLED?'<div class="driver-preview"><strong>Prévia funcional.</strong> A autenticação HttpOnly, os eventos e as ações reais serão ligados pelo adapter.</div>':''}${driverState.screen==='queue'?renderDriverQueue(route):driverState.screen==='history'?renderDriverHistory() : renderDriverProfile()}</main>${renderDriverNav()}</div>`;}
...
async function advanceDriverDelivery(deliveryId,command,version){if(driverState.busy.has(deliveryId))return;if(command==='complete'){openDriverCompletion(deliveryId,version);return}driverState.busy.add(deliveryId);const body={expected_version:version};try{if(!driverState.online&&DRIVER_API_ENABLED){driverQueueCommandOffline(deliveryId,command,body);driverApplyLocalCommand(deliveryId,command);navigator.vibrate?.(45);driverToast('Ação salva. Será sincronizada quando a internet voltar.');return}await driverGateway.command(deliveryId,command,body);await loadDriverQueue();navigator.vibrate?.(45);driverToast({pickup:'Retirada confirmada.',start:'Rota iniciada.',arrive:'Chegada informada.'}[command]||'Entrega atualizada.')}catch(error){driverToast(error.message)}finally{driverState.busy.delete(deliveryId)}}
...
async function submitDriverIncident(deliveryId){const reason=document.getElementById('driver-incident-reason')?.value.trim();const error=document.getElementById('driver-incident-error');if(!reason){error.textContent='Descreva o problema para a expedição.';return}try{if(!driverState.online&&DRIVER_API_ENABLED){driverQueueCommandOffline(deliveryId,'incident',{reason});driverApplyLocalCommand(deliveryId,'incident');closeDriverModal();driverToast('Ocorrência salva. Será enviada quando a internet voltar.');return}await driverGateway.command(deliveryId,'incident',{reason});closeDriverModal();await loadDriverQueue();driverToast('Ocorrência enviada à expedição.')}catch(requestError){error.textContent=requestError.message}}
function closeDriverModal(){document.getElementById('driver-modal')?.remove()}
async function logoutDriverPortal(){if(driverState.locationWatchId!==null){navigator.geolocation?.clearWatch(driverState.locationWatchId);driverState.locationWatchId=null}driverSetLocationEnabled(false);await driverGateway.logout().catch(()=>{});sessionStorage.removeItem('clickgarcom_driver_preview');driverState.session=null;driverState.assignments=[];driverState.history=[];driverState.demoQueueInitialized=false;if(driverState.timer)clearInterval(driverState.timer);driverState.timer=null;renderDriverLogin()}
...
function bindDriverConnectivity(){window.addEventListener('online',async()=>{driverState.online=true;await flushDriverPendingCommands();await flushDriverPendingLocations();loadDriverQueue()},{passive:true});window.addEventListener('offline',()=>{driverState.online=false;renderDriverApp()},{passive:true});if(!driverState.timer)driverState.timer=setInterval(()=>{if(driverState.online&&driverState.screen==='queue'){flushDriverPendingCommands();flushDriverPendingLocations();loadDriverQueue()}},15000)}
bootDriverPortal();
